...
prompt_template = ChatPromptTemplate.from_messages([
    ("system", """
        ...
        Gib die 5 relevantesten Filme oder Serien an.
        {format_instructions}
    """),
...
# %% chain invocation
st.title("Wie hieß nochmal der Film?")
 
filmbeschreibung = st.chat_input("Gib hier eine Filmbeschreibung ein!")
if filmbeschreibung:
    ...
    with st.chat_message("ai"):
        for r in res.model_dump()["movies"]:
            ...
            st.markdown(f"**Schauspieler*innen:** {r['actors']}")
            st.divider()
            # st.write(r['actors'])
